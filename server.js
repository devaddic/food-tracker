...
app.post('/fetchEndLife', async (req, res) => {
  ...
  const payload = {
    prompt: `Answer by only stating a number + "days", "weeks","months", "years". Let say I have ${foodItem} that is kept in ${storageType} and was fresh since the unix time in seconds: ${startLife}. When can I expect it to expire? Or if it is not a food item state exactly the word "invalid".`
  };
